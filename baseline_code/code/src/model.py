...
class CrossStockAttention(nn.Module):
    """股票间交互注意力模块"""
    def __init__(self, d_model, nhead, dropout=0.1, residual_scale=0.5):
    ...
    def forward(self, stock_features):
        ...
        output = self.norm(stock_features + self.residual_scale * self.dropout(attended))
        return output
...
class StockTransformer(nn.Module):
    def __init__(self, input_dim, config, num_stocks, emb_dim=16):
        ...
        # 时序特征提取
        self.temporal_residual_scale = nn.Parameter(
            torch.tensor(float(config.get('temporal_residual_scale', 0.5)))
        )
        ...
        self.cross_stock_attention = CrossStockAttention(
            ...
            config['nhead'],
            config['dropout'],
            residual_scale=config.get('cross_residual_scale', 0.5),
        )
        ...
        # === 辅助任务头（获奖队伍 7355608 推荐） ===
        # 方向预测：未来5日是否上涨（二分类）
        self.use_aux_direction = config.get('use_aux_direction', False)
        if self.use_aux_direction:
        ...
        # 波动率预测：未来5日波动率回归
        self.use_aux_volatility = config.get('use_aux_volatility', False)
        if self.use_aux_volatility:
    ...
    def forward(self, src, return_aux=False):
        ...
        # Strategy variants can enable this to block future-token attention.
        causal_mask = None
        if self.config.get('use_causal_temporal_mask', False):
            causal_mask = torch.triu(
                torch.ones(seq_len, seq_len, device=src_proj.device, dtype=torch.bool),
                diagonal=1,
            )
        temporal_encoded = self.temporal_encoder(src_proj, mask=causal_mask)  # [batch*num_stocks, seq_len, d_model]
        temporal_features = src_proj + self.temporal_residual_scale * (temporal_encoded - src_proj)
        ...
        if return_aux:
            aux = {}
            if self.use_aux_direction:
                aux['direction'] = self.direction_head(ranking_features).view(batch_size, num_stocks)
            if self.use_aux_volatility:
                aux['volatility'] = self.volatility_head(ranking_features).view(batch_size, num_stocks)
            return output, aux
