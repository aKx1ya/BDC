...
ADDITIONAL_TIME_FACTOR_COLUMNS = [
    'vwap_close_gap', 'vwap_open_gap', 'log_turnover', 'log_volume_rate',
    'mom5_vol10', 'mom10_vol20', 'mom20_vol20',
]
ADDITIONAL_CROSS_SECTIONAL_COLUMNS = [
    'return_5_cs_rank', 'return_5_cs_z',
    'return_10_cs_rank', 'return_10_cs_z',
    'volume_ratio_cs_rank', 'volume_ratio_cs_z',
    'vwap_close_gap_cs_rank', 'vwap_close_gap_cs_z',
    'log_turnover_cs_rank', 'log_turnover_cs_z',
]
ADDITIONAL_FACTOR_COLUMNS = ADDITIONAL_TIME_FACTOR_COLUMNS + ADDITIONAL_CROSS_SECTIONAL_COLUMNS


def _add_time_safe_factors(df, open_, close, volume, amount, turnover, enabled=None):
    """Add current/past-only factors; no future rows are referenced."""
    if enabled is None and os.getenv('BDC_ENABLE_EXTRA_FACTORS', '0') != '1':
        return df
    if enabled is False:
        return df
    eps = 1e-12
    ...
    df['vwap_close_gap'] = (vwap - close) / (close + eps)
    df['vwap_open_gap'] = (vwap - open_) / (open_ + eps)
    df['log_turnover'] = np.log1p(turnover.clip(lower=0))
    df['log_volume_rate'] = np.log1p(volume / (volume.shift(1) + eps))
    df['mom5_vol10'] = close.pct_change(5) / (return_1.rolling(10).std() + eps)
    df['mom10_vol20'] = close.pct_change(10) / (return_1.rolling(20).std() + eps)
    df['mom20_vol20'] = close.pct_change(20) / (return_1.rolling(20).std() + eps)
    return df


def add_extra_factor_features(df):
    df = df.copy()
    ...
    for _, group in df.groupby('股票代码', sort=False):
        ...
        enriched.append(_add_time_safe_factors(group, open_, close, volume, amount, turnover, enabled=True))
    out = pd.concat(enriched).reset_index(drop=True)
    return add_cross_sectional_features(out)


def add_cross_sectional_features(df):
    """
    Add same-date cross-sectional ranks/z-scores using only stocks tradable on
    that date. This avoids future survivor statistics and keeps factors causal.
    """
    df = df.copy()
    df['日期'] = pd.to_datetime(df['日期'])
    base_columns = ['return_5', 'return_10', 'volume_ratio', 'vwap_close_gap', 'log_turnover']
    for col in base_columns:
        ...
        df[f'{col}_cs_z'] = (df[col] - mean) / (std + 1e-12)
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    for col in ADDITIONAL_CROSS_SECTIONAL_COLUMNS:
        if col not in df.columns:
            df[col] = 0.0
    df[ADDITIONAL_CROSS_SECTIONAL_COLUMNS] = df[ADDITIONAL_CROSS_SECTIONAL_COLUMNS].fillna(0.0)
    return df
...
def engineer_features_39(df):
    ...
    df = _add_time_safe_factors(df, open_, close, volume, amount, turnover)
...
def create_ranking_dataset_vectorized(
    ...
    # 3. 为每只股票生成所有滑动窗口。
    # label 已在全量交易日序列中对齐为 T+1 open 到 T+6 open，
    # 这里不再要求自然日连续，避免周末和节假日错误删除交易日样本。
    ...
    for stock_code, group in tqdm(grouped, desc="Processing stocks"):
        ...
        for i in range(num_windows):
            ...
            target = labels[end_idx]                        # label 对应窗口最后一天的 T+1/T+6 开盘收益
    ...
    allowed_dates = None
    if allowed_window_end_dates is not None:
        allowed_dates = {
            pd.Timestamp(date).normalize()
            for date in allowed_window_end_dates
        }
    
    for date, group in tqdm(grouped_by_date, desc="Aggregating by date"):
        date_key = pd.Timestamp(date).normalize()
        if min_window_end_date is not None and date_key < min_window_end_date:
            continue
